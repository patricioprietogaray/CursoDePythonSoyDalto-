...
# Enunciado Simple: 
# Define dos clases diferentes que tengan un método llamado ejecutar(). 
# Crea una función que tome 
# un objeto como argumento y llame a su método ejecutar(). 
# Demuestra cómo puedes pasar instancias de ambas clases a esta función 
# y se ejecutará el método correspondiente de cada objeto.

class Commodore64:
    def ejecutar(self):
        return 'El codigo se ejecuta en un Commodore 64'
    
class CZ1500:
    def ejecutar(self):
        return 'El código se ejecuta en un CZ1500...'

def se_ejecuta(compu):
    # compu.ejecutar()  # asi retorna None    FALTA RETURN
    return compu.ejecutar()


while True:
    opc = int(input('ingrese cero para salir: '))
    if opc == 0:
        break
    ...
    print(se_ejecuta(compu1))
    print(se_ejecuta(compu2))
